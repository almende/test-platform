...
function createNewCardTemplate (vApp) {
    ...
    var vApp_name = vApp.name,
        vApp_link = 'http://localhost' + frontendUri,
...
function cleanFrontendUri(frontendUri){
    ...
    // for frontendUri labels like "localhost:8081/" we need to remove the first part
    var separatorPos = frontendUri.indexOf(":");
    if(separatorPos > -1){
        newFrontendUri = frontendUri.slice(separatorPos);
    }
